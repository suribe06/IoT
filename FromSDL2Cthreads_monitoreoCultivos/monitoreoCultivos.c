...
static void *pConsole ( void *arg_ptr ){
  ...
  printf ( "Bienvenido al Monitoreo de Invernadero!\n" );
  ...
  while ( timerActive != 0 ){
    ...
    switch ( state ){
      ...
      case CheckTimeout:
        switch ( InMsg.signal ){
          case sTimeout:
            OutMsg.signal = (int) sT;
            OutMsg.value1 = 0;
            sendMessage ( &(queue [CONSOLE_Q]), OutMsg );        /* send message to pConsole */
            state_next = IdleT;
            break;
          case sResetTimer:
