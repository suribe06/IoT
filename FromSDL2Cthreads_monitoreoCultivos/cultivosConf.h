...
#define NUM_QUEUES  16     /* number of queues */
...
#define TIMER_Q               15     /* queue 15: timer process */
...
typedef enum{
  sSendData,
  sT
} TO_CONSOLE; /* Signals of pConsole*/
...
typedef enum{
  ...
  sSendTemp,
  sSendPH
} TO_CONTROLLER; /* Signals of pController*/

typedef enum{
  sGetHum
} TO_SENSOR_H; /* Signals of sensors pSenHumidity */

typedef enum{
  sGetTemp
} TO_SENSOR_T; /* Signals of sensors pSenTemperature */

typedef enum{
  sGetPH
} TO_SENSOR_PH; /* Signals of sensors pSenPH*/
...
typedef enum{
  ...
  sResetTimer,
  sKillTimer
} TO_TIMER; /* Signals sent to timer */
